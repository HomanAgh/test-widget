...
export async function GET(req: NextRequest) {
  ...
  if (!leagueSlug) {
    console.log("Missing leagueSlug query parameter"); // Log missing parameter
  ...
  if (!apiKey || !apiBaseUrl) {
    console.log("Missing API key or base URL"); // Log missing environment variables
    return NextResponse.json(
  ...
  const standingsUrl = `${apiBaseUrl}/v1/leagues/${leagueSlug}/standings?offset=0&limit=100&sort=group&apiKey=${apiKey}`;
  console.log("Fetching league standings from:", standingsUrl); // Log API URL
  ...
  try {
    ...
    return NextResponse.json(data); // Return the full standings data
  } catch (error: any) {
    console.error("Error during fetch:", error.message); // Log errors
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  ...
  const fields = "team.name,team.links.eliteprospectsUrl,stats.GP,stats.W,stats.L,stats.PTS,season.slug";
