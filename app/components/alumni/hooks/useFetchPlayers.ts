  ...
  async function fetchPlayers(reset = false) {
    ...
    try {
      ...
      const players: AlumniPlayer[] = data.players.map((player) => ({
        id: player.id,
        name: player.name,
        birthYear: player.birthYear ?? null, // Using nullish coalescing
        gender: player.gender ?? undefined, // Ensure gender is string | undefined
        teams: [],
      }));
      ...
      console.log("Players with Extra Data:", playersWithExtra); // Verify gender is intact
...
export function useFetchPlayers(
  ...
  const normalizeGender = (gender?: string | null): string | undefined => {
    ...
    if (lower === 'men') return 'male';
    if (lower === 'women') return 'female';
  ...
  async function fetchPlayers(reset = false) {
    ...
    try {
      ...
      const players: AlumniPlayer[] = data.players.map((player) => ({
        ...
        gender: normalizeGender(player.gender), // Normalize gender
        teams: [],
      }));
