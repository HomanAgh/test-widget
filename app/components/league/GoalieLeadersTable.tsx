...
const GoalieLeadersTable: React.FC<GoalieLeadersTableProps> = ({
  ...
  const sortedData = [...goalieLeaders.data].sort((a, b) => {
    ...
    switch (sortColumn) {
      ...
      case "GP":
        ...
        break;
      case "GAA":
        result = (getPlayerStats(a)?.GAA || 0) - (getPlayerStats(b)?.GAA || 0);
        break;
      case "SVP":
        result = (getPlayerStats(a)?.SVP || 0) - (getPlayerStats(b)?.SVP || 0);
        break;
      default:
    ...
    if (sortDirection === "none") {
      return (getPlayerStats(b)?.SVP || 0) - (getPlayerStats(a)?.SVP || 0);
    }
  ...
  const initialRanking = [...goalieLeaders.data]
    .sort(
      (a, b) => (getPlayerStats(b)?.SVP || 0) - (getPlayerStats(a)?.SVP || 0)
    )
    .map((goalie, index) => {
      // Ensure we have a valid ID for matching
      const goalieId =
        goalie.id ||
        `${goalie.player.firstName}-${goalie.player.lastName}-${goalie.team.name}`
          .toLowerCase()
          .replace(/\s+/g, "-");
  ...
  const rankedData = sortedData.map((goalie) => {
    ...
    if (!rankInfo) {
      rankInfo = initialRanking.find(
        (r) =>
          r.playerName ===
          `${goalie.player.firstName} ${goalie.player.lastName}`
      );
    }
            ...
            <TableRow key="header-row" bgColor={customColors.backgroundColor}>
              {headerCells.map((cell) => (
                <TableCell
                  ...
                  onClick={() =>
                    handleSort(cell.key === "rank" ? "SVP" : cell.key)
                  }
                >
                  <span>{cell.label}</span>{" "}
                  {renderSortArrow(cell.key === "rank" ? "SVP" : cell.key)}
                </TableCell>
              ))}
            </TableRow>
          </TableHead>
          <TableBody>
            {paginatedData.map((goalie, index) => {
              const nationalityName = getNationalityName(
                goalie.player.nationality
              );

              // Prioritize EliteProspects URL for player links
              const playerUrl =
                goalie.player.links?.eliteprospectsUrl ||
                (goalie.player.id
                  ? `https://www.eliteprospects.com/player/${
                      goalie.player.id
                    }/${goalie.player.lastName?.toLowerCase()}-${goalie.player.firstName?.toLowerCase()}`
                  : goalie.player.slug
                  ? `/player/${goalie.player.slug}`
                  : "#");

              const teamUrl =
                goalie.team.links?.eliteprospectsUrl ||
                (goalie.team.slug
                  ? `/team/${goalie.team.slug}`
                  : `#team-${goalie.team.id}`);

              // Create a unique key using index as fallback when id is undefined
              const uniqueKey = goalie.id
                ? `goalie-${goalie.id}`
                : `goalie-index-${index}`;

              // Get the right stats based on the statsType
              const playerStats = getPlayerStats(goalie);
              ...
              return (
                ...
                >
                  ...
                  <TableCell key={`player-${uniqueKey}`} align="left">
                    <div className="flex items-center">
                      {goalie.player.flagUrl && (
                        <div
                          key={`flag-container-${goalie.player.id || index}`}
                      ...
                      <div key={`player-name-${goalie.player.id || index}`}>
                        ...
                        >
                          {typeof goalie.player.firstName === "string"
                            ? goalie.player.firstName
                            : ""}{" "}
                          {typeof goalie.player.lastName === "string"
                            ? goalie.player.lastName
                            : ""}
                        </Link>
                      </div>
                    </div>
                  ...
                  <TableCell key={`team-${uniqueKey}`} align="left">
                    <div className="flex items-center">
                      {goalie.team.logo?.small && (
                        <div
                          key={`logo-container-${goalie.team.id || index}`}
                          className="flex-shrink-0 mr-2"
                        >
                          <img
                  ...
                  <TableCell key={`gp-${uniqueKey}`} align="center">
                    {playerStats?.GP || 0}
                  </TableCell>
                  <TableCell key={`gaa-${uniqueKey}`} align="center">
                    {formatGAA(playerStats?.GAA)}
                  </TableCell>
                  <TableCell key={`svp-${uniqueKey}`} align="center">
                    {formatSVP(playerStats?.SVP)}
                  </TableCell>
                </TableRow>
              );
            })}
