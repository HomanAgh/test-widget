...
const NationalityFilter: React.FC<NationalityFilterProps> = ({
  ...
  const allSelected = NATIONALITIES.every((option) =>
    selectedValues.includes(option.value)
  );

  const handleSelectAllToggle = () => {
    onSelectionChange(allSelected ? [] : NATIONALITIES.map((opt) => opt.value));
  };
    ...
    <div className="relative">
      ...
      {selectedValues.length > 0 && (
        <div className="mt-2 p-2 rounded text-[#052D41] font-montserrat text-lg">
          <strong>Selected Nationalities</strong>
          <div className="flex flex-wrap gap-2 mt-3 mb-2">
            {selectedValues.map((value) => {
              const option = NATIONALITIES.find((opt) => opt.value === value);
              return (
                <span
                  key={value}
